...
def upsert_document(doc: DocumentRecord) -> None:
    with _conn() as conn:
        conn.execute(
            """INSERT INTO documents (document_id, filename, page_count, fact_count, status, ingested_at)
               VALUES (?, ?, ?, ?, ?, ?)
               ON CONFLICT(document_id) DO UPDATE SET
                 fact_count=excluded.fact_count,
                 status=excluded.status""",
            (doc.document_id, doc.filename, doc.page_count,
             doc.fact_count, doc.status, doc.ingested_at),
        )
...
def delete_document(document_id: str) -> int:
    with _conn() as conn:
        ...
        fact_rows = conn.execute(
            "SELECT fact_id FROM facts WHERE document_id = ?", (document_id,)
        ).fetchall()
        ...
        if fact_ids:
            ...
            for fid in fact_ids:
                conn.execute(
                    "DELETE FROM relationships WHERE fact_ids_json LIKE ?",
                    (f"%{fid}%",),
                )
    return len(fact_ids)
...
def insert_facts(facts: List[Fact]) -> None:
    ...
    with _conn() as conn:
        conn.executemany(
            """INSERT INTO facts
               (fact_id, document_id, source_filename, page_number, chunk_id,
                entity, metric_or_claim, value, unit, time_period, context,
                exact_quote, confidence, grounded, extra_json)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            [
                (
                    f.fact_id, f.document_id, f.source_filename, f.page_number,
                    f.chunk_id, f.entity, f.metric_or_claim, f.value, f.unit,
                    f.time_period, f.context, f.exact_quote, f.confidence,
                    int(f.grounded), json.dumps(f.extra),
                )
                for f in facts
            ],
        )
    ...
    with _conn() as conn:
        conn.executemany(
            """INSERT INTO extraction_failures
               (failure_id, document_id, source_filename, page_number, chunk_id, reason, raw_excerpt)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            [
                (f.failure_id, f.document_id, f.source_filename,
                 f.page_number, f.chunk_id, f.reason, f.raw_excerpt)
                for f in failures
            ],
        )
...
def relationship_exists(cluster_key: str) -> bool:
    with _conn() as conn:
        row = conn.execute(
            "SELECT 1 FROM relationships WHERE cluster_key = ?", (cluster_key,)
        ).fetchone()
    ...
    with _conn() as conn:
        conn.execute(
            """INSERT OR IGNORE INTO relationships
               (relationship_id, relationship_type, fact_ids_json,
                explanation, resolution_context, cluster_key)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                rel.relationship_id, rel.relationship_type.value,
                json.dumps(rel.fact_ids), rel.explanation,
                rel.resolution_context, rel.cluster_key,
            ),
        )
...
def get_stats() -> SystemStats:
    with _conn() as conn:
        ...
        grounded = conn.execute("SELECT COUNT(*) FROM facts WHERE grounded = 1").fetchone()[0]
        ungrounded = total_facts - grounded
        total_failures = conn.execute("SELECT COUNT(*) FROM extraction_failures").fetchone()[0]
        ...
        rels_by_type: Dict[str, int] = {r["relationship_type"]: r["cnt"] for r in rel_rows}
